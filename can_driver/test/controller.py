...
class SerialBus(BusABC):
    ...
    def generate_checksum(self, data, data_len):
        checksum = 0
        for i in range(data_len):
                checksum += data[i]
        return checksum & 0xff

    def send_cmd(self, msg=None):
        byte_msg = bytearray()
        byte_msg.append(0xaa)
        byte_msg.append(0x55)
        byte_msg.append(0x12)
        byte_msg.append(0x05)#spped
        byte_msg.append(0x01)#frame
        ...
        byte_msg.append(0x00)
        byte_msg.append(0x00)
        byte_msg.append(0x00)# mode
        byte_msg.append(0x01)
        byte_msg.append(0x00)
        byte_msg.append(0x00)
        byte_msg.append(0x00)
        byte_msg.append(0x00)
        checksum = self.generate_checksum(byte_msg[2:], 17)
        byte_msg.append(checksum)


        # byte_msg.append(msg.arbitration_id & 0x00ff) # arbitration_id low 
        # byte_msg.append((msg.arbitration_id >> 8) & 0x00ff) # arbitration_id high
        # byte_msg.append((msg.arbitration_id >> 16) & 0x00ff) # arbitration_id high
        # byte_msg.append((msg.arbitration_id >> 24) & 0x00ff) # arbitration_id high
        # for i in range(msg.dlc):
        #     byte_msg.append(msg.data[i])
            # print(hex(msg.data[i]))
        # print("\n")
        # byte_msg.append(0x55)
        print('SEND CMD', byte_msg)
    ...
    def send(self, msg):
        ...
        # #     byte_msg.append(a_id[i])
           # # byte_msg.append(0x20)
           # # byte_msg.append(0x00)
        # for i in range(0, msg.dlc):
        #     byte_msg.append(msg.data[i])
        # byte_msg.append(0x55)

        # print(byte_msg, len(byte_msg))
        # self.ser.write(byte_msg)
...
class Controller:
    ...
    def set_speed(self, speed:int):
        ...
        self.cmd_data[1] = 0x00
        self.cmd_data[2] = 0x07
        self.cmd_data[3] = 0x00
        self.cmd_data[4] = 0x00
        self.cmd_data[5] = 0x00
        self.cmd_data[6] = 0x00
        self.cmd_data[7] = 0x00
        msg = Message(
